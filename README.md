...
metadata/
  ...
  company-registry.json # A 股主板公司观察池注册表
...
公司状态用于描述核心标的观察池中的 A 股主板上市公司，维护在 `metadata/company-registry.json`：

- `candidate`：候选标的，已证明方向相关，但收入承接、订单或利润弹性尚未充分验证；
- `tracking`：持续跟踪，已有方向关联和业务承接证据；
- `validated`：已被订单、收入、利润、客户认证或产能证据验证；
- `weakened`：逻辑削弱，方向匹配、收入承接或利润弹性被新证据削弱；
- `rejected`：已证伪，仅剩概念相关或明确无法承接方向收入；
- `archived`：归档，仅保留历史。

核心标的观察池只记录方向匹配、收入承接、验证信号和风险，不输出买入、卖出、目标价或仓位建议。
...
当前项目已经配置知识库更新闭环。日频、周频、月频和季度任务均已完成首次运行，实际运行结果以 `metadata/update-state.json` 为准：
...
- 周频任务可以把公司候选升级或调整到核心标的观察池，但必须保留来源和验证信号；
- 月频任务更新观察清单，并同步公司层跟踪信号和观察优先级；
- 季度任务更新复盘记录，并复盘公司是否真正验证方向判断；
- 评分模型暂不启用；等核心标的观察池运行 1-2 个季度并形成复盘样本后，再考虑加入方向匹配度、兑现确定性、定价拥挤度和风险权重；
...
- 公司注册表中的公司状态、交易所、板块、方向引用和来源层级是否有效；
- 来源注册表中的来源层级是否有效；
